...
## 2026-08-14 01:05 · T12.3 增强 · 验证控制与完成门槛

触发的流程：

- 我继续复审反馈循环的停止条件，确认模型可以在验证失败或代码写入后直接声明 `finish`，也没有独立的验证预算和重复失败熔断。
- 我创建 `feat/t12-validation-controls`，选择依据实际工具事件维护确定性状态，不通过任务文本关键词猜测是否需要验证；Codex 协助 TDD、实现和边界复审。

TDD 与设计判断：

- 第一轮相关测试 `4 failed, 4 passed`，准确证明两条假成功路径以及预算、熔断参数缺失。
- 成功写入代码或工程配置会设置“需要重新验证”；验证失败设置“验证尚未通过”；只有新的客观验证通过才能清除完成门槛。
- 普通文本写入不强制验证，未配置 Validator 的库级用法保持工具 observation 语义，避免扩大机制适用范围。
